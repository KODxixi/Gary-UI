...
背景只绘制点，不叠加线网、卡片等高线或第二套点阵。Canvas 成功后隐藏 CSS 点阵；嵌入页保留独立的不透明页面底色。
...
- DeepSeek：中性半透明底板与独立边缘分层，只有链接/按钮有悬停、焦点和按压反馈。
  两个主题共用 `--gary-card-*` token；主卡仍为 30px。局部轮廓 Canvas 是 Gary 原创绘制，不使用远程 shader。
- COSMOS 本地镜像：安静的首屏中心、轻字重展示标题、充分章节间隔、内容本身作为展示主体。
  在 Demo 首页和展示情境使用 400 字重；中文不照搬英文的负字距，不加入镜像照片、视频或专有字体。
- 网格：28px 点阵 + 90px 低对比网格，指针仅影响邻域；停止输入不持续绘制，减弱动态直接显示完整静态图形。
